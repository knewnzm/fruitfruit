...
<div>
	<table  class="list">
		<tbody>
			<tr>
			<th>주문 상태</th><th>구매자 이름</th> <th>구매 수량</th> <th>구매자 주소</th> <th>결제 금액</th> 
			</tr>
...
<c:if test="${o.order_type == 3}">
										...
										onclick="del(${o.order_num},${p.product_num})">목록에서 삭제</button> 
										</c:if>
										<c:if test="${o.order_type == 0}">
				<button class="comple_btn" 
				onclick="location.href=`${pageContext.request.contextPath}/order/change?order_num=${o.order_num}&product_num=${p.product_num }`">주문수락</button></td>
</c:if>
